...
export async function createChat(
  ...
  await sql`INSERT INTO chats (user_name, name) VALUES (${userName}, ${name})`;
...
export async function getChats(userName: string): Promise<Chat[]> {
  const { rows: chats } =
    await sql`SELECT * FROM chats WHERE user_name = ${userName}`;
...
export async function getChatsWithMessages(
  ...
  const { rows: chats } =
    await sql`SELECT * FROM chats WHERE user_name = ${userName} ORDER BY timestamp DESC LIMIT 3`;
